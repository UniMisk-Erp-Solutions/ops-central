...
Deno.serve(async (req: Request) => {
  ...
  if (path === "/float-rfq" && req.method === "POST") {
    ...
    // Each vendor may carry its OWN item list (per-item vendor selection); fall back
    // to the shared items when a vendor sends none.
    const cleanOf = (arr: any[]) => arr.map((i: any) => ({ product_id: i.product_id, name: i.name || i.product_id, code: i.code || "", qty: Number(i.qty) || 0 }));
    const rfqVendors = withEmail.map((v: any) => ({ vendor_id: v.vendor_id, name: v.name || "Vendor", email: v.email, token: rfqId + "~" + rand(), items: (Array.isArray(v.items) && v.items.length) ? cleanOf(v.items) : cleanItems, prices: {}, status: "sent", sent_at: new Date().toISOString() }));

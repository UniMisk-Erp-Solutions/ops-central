...
function ProcurementTab({ so }) {
  ...
  // directly from the picks and prices already on it. This is also how a
  // request-converted order gets its RFQ at all: convert() links a Sourcing
  // record (see docs/client-requests.md and docs/so-float-rfq.md) purely as
  // an internal vendor-comparison workspace, not a second customer-facing
  // inquiry — Purchase never converts it a second time.
  //
  // 'Draft' is allowed alongside the historic 'Approved'/'Procurement
  // Started' because that is exactly where a converted request's SO sits
  // until its first Vendor PO exists (see docs/client-requests.md's note on
  // the approval stage being bypassed by design for that flow) — every
    ...
    <div className="stack">
      ...
      {canProcure && (
        <div className="card"><div className="card-body" style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div className="grow"><strong className="small">Procure for this SO</strong><div className="tiny muted">{sourcing ? 'Or skip comparison — pick a vendor and fill in prices yourself.' : 'Raise a Vendor PO for this order.'}</div></div>

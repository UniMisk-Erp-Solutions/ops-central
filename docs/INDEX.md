...
| [keyboard-control.md](./keyboard-control.md) | Working without a mouse | Every screen from the keyboard: Tab reaches every control, up/down drive the list, left/right cross between sidebar and page, `]`/`[` and `1`-`9` for the tabs on a record, the command palette, `g` jumps, dialog focus traps, and the rule that nothing fires while you are typing. |

...
| [keyboard-control.md](./keyboard-control.md) | Working without a mouse | Every screen from the keyboard: Tab reaches every control, a list is one tab stop the arrows drive, the command palette, `g` jumps, dialog focus traps, and the rule that the arrows belong to whatever has focus. |

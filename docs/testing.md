...
| `so-rfq-check` | that the edge function resolves an organization from the SO too (not only a Sourcing record), and only as a fallback after the existing Sourcing lookup; that `createPOFromRFQQuote` builds the exact same PO shape `CreateVendorPOModal` does — MD threshold, `Procurement Started` advance, and a vendor who has not priced anything yet is refused rather than given a ₹0 PO; and that the panel steps aside whenever a Sourcing record already exists for the SO, leaving the manual "Create Vendor PO" path untouched | dm's Purchase team has no Sourcing/inquiry step in front of an SO at all (see `client-requests-check`), so Float RFQ — done by Pre-sales in the main flow — had nowhere to run from; built to reuse the exact same edge function, vendor-quote link and `config.vendor_emails` Sourcing's Float RFQ already uses, not a second implementation |

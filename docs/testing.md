...
| `keyboard-check` | that no shortcut fires while somebody is typing, that the browser keeps its own, row movement and its ends, that every clickable div is reachable by Tab while a row's children are not, that up/down drive the list while Home/End stay with the page until a row is picked, that focus can always get back out of the sidebar, that nothing touches browser history, and that the palette cannot offer a screen the sidebar hides | a stray key mid-entry loses an accountant's work, and a palette is a permissions hole if it is a second list |

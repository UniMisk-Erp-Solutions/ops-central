...
| `client-review-check` | that a decided unit never returns to pending, that a decision clamps to what is actually pending rather than what was asked for, that the two new lifecycle stages sit strictly between Fully Delivered and Invoiced without moving any other status pair, and that the whole feature is invisible with the workflow key off | soDerivedStatus called wfOn() unguarded and crashed a check that sandboxes utils.jsx alone -- one bug that would have failed every status assertion in the suite, not only the new ones |
| `roles-check` | that an unrecognised role is NOT an administrator, that who may receive comes from the workflow profile rather than a literal, that the `standard` profile still yields exactly the four roles it always did, and that every role Settings offers is one the app defines | `perm()` fell back to Org Admin for any unknown role, and Settings offered `Project Management` while the key was `Project Manager` — so picking it from the dropdown handed out full administrative access, silently |

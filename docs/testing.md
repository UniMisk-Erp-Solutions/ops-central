...
```bash
for t in render boot import alloc pricing receive receipt-engine dispatch-invoice status numbering boq keyboard reach roles client-review client-requests dm-ux so-rfq dispatch-queue; do
...
| `dispatch-invoice-check` | customer wording, partial→final adding to the order value, no double billing, the over-dispatch cap; and that a client-review rejection's replacement dispatch — on an order already fully invoiced the first time round — raises no second invoice | a rejection's replacement is a different physical unit for the same already-billed order value, not a new sale; the same "capped at what is still uninvoiced" logic that stops an over-dispatch from over-billing also stops this from double-billing, with nothing rejection-specific added |
...
| `client-review-check` | that a decided unit never returns to pending, that a decision clamps to what is actually pending rather than what was asked for, that the two new lifecycle stages sit strictly between Fully Delivered and Invoiced without moving any other status pair, that a rejection's "ordered" and "fulfilled" states are two different figures gated to two different consumers, that the extra quantity lands on exactly one row when a product spans several, that the whole feature is invisible with the workflow key off; and, for the linked-Sourcing path specifically, that `vendorPOGroups` offers exactly the rejected quantity on a second call — never the full original amount again, never doubled with it — reusing the same vendor already picked, that the replacement Vendor PO closes `soRejectedOutstanding` back to empty, and that Purchase gets a real Inbox task (not only a one-time notification) that clears itself once re-ordered | soDerivedStatus called wfOn() unguarded and crashed a check that sandboxes utils.jsx alone; gating a close on "has Purchase ordered a replacement" instead of "has the client accepted it" would have let an order close while still owing a unit; and a rejection was recorded correctly but had nowhere to go from there — `vendorPOGroups` never netted against existing Vendor POs at all, so the SO Procurement tab's own re-generate button would have raised the *entire original order* again, not the replacement, the moment `canGenerate`'s blunt `linkedPOs.length === 0` gate was loosened to let it run a second time |
...
| `client-requests-check` | that Client Facing's base capabilities carry no `createSO`/`editOwnDraft`, that Purchase's only door into creating an SO is `convertClientRequest`, that the whole route is invisible with `client_order_requests` off (same discipline as `client_acceptance`), that recommendations rank by frequency then recency and never leak across customers, that document numbering follows the one shared scheme, that a converted SO's line `unit_price` is a real rollup of its components rather than 0, and that every invoice-building function checks `soReviewStillOpen` before forcing an order's status to `Invoiced` | the client-creates-the-SO design was reversed mid-project — the sheet importer briefly admitted Client Facing, and Client Facing's base `can` briefly carried `createSO` — and a role's `primary` route pointing at a workflow-gated route it could not actually open broke `roles-check`'s "no role lands on a screen it may not open"; and an actual end-to-end dry run (client request → convert → vendor PO → GRN → dispatch) found the converted order could never be invoiced at all (`unit_price: 0` on every line), and that once it could, the status strip jumped straight to "Invoiced," skipping the review stage the organization had switched on |
...
| `so-rfq-check` | that `convert()` links a real Sourcing to the new SO (`converted_so_id`) rather than building a second RFQ implementation; that its status is never `'Converted'`, so every action button on the Sourcing screen stays live; that the SO's Procurement tab finds it and offers one-click PO generation; that `generateVendorPOsFromSourcing` raises one PO per vendor, each priced from what was picked on that vendor, for that vendor's items only; that "Send to Sales" is replaced by "Create Vendor PO(s)" exactly when `converted_so_id` is set (never a hard-coded tenant check), saving picks first so nothing typed is lost; and that every hook `SourcingDetail` owns is declared before its `if (!src) return <div>...` guard, with none after it | dm's Purchase team has no Sourcing/inquiry step in front of an SO at all (see `client-requests-check`); rather than a second, thinner RFQ panel (the first attempt, later removed), a real Sourcing record is linked so Purchase gets the exact per-item vendor comparison, Float RFQ and one-click PO screen Pre-sales already has; a hook added after the component's early return once shipped ("Rendered more hooks than during the previous render," crashing the page on every refresh) — section [7] statically fails if that ordering regresses |
| `dispatch-queue-check` | that `scmLineTotals` correctly separates an order with stock in the Virtual Godown from one with none; that `ReadyToDispatchPanel` lists the former with the right customer, items and a Dispatch shortcut, and never lists the latter; that Stores/Purchase/Org Admin/Managing Director/Stores In — every role that already had a working "Out for delivery" path — see no new panel and their existing button + table are byte-for-byte unchanged; and that a role with no dispatch capability at all (Sales) sees neither | Stores Out had the dispatch capability and the modal to use it, but nothing told them WHICH order had stock ready to go — SCM Tracking's dropdown defaults to whichever order loads first, so a dispatch-only role with no reason to browse Sales Orders could open the screen, land on an unrelated order, and conclude the feature did not exist; scoped to `role === 'Stores Out'` rather than an org check so no other organization's screen — Microlink included — changes at all |
